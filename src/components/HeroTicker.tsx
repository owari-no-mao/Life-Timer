...
import { Hourglass, Sun, Calendar, Zap, Clock } from 'lucide-react';

interface HeroTickerProps {
  profile: UserProfile;
}

type TimeUnit = 'detailed' | 'months' | 'weeks' | 'days' | 'hours' | 'minutes' | 'seconds';

export const HeroTicker: React.FC<HeroTickerProps> = ({ profile }) => {
  ...
  // Update clock once every 100ms so tenth of seconds tick smoothly
  useEffect(() => {
  ...
  const renderRemainingDisplay = () => {
    ...
    switch (selectedUnit) {
      ...
      case 'months': {
        const totalMonths = days / (365.2425 / 12);
        return {
          main: formatLargeNumber(Math.floor(totalMonths)),
          unit: 'ヶ月',
          label: showWakingOnly ? '残された覚醒月数（睡眠除く）' : '残された総月数',
        };
      }
      case 'weeks': {
        const wks = days / 7;
      ...
      case 'seconds': {
        ...
        const integerPart = formatLargeNumber(Math.floor(secs));
        const decimalDigit = Math.floor((secs % 1) * 10);
        return {
          main: `${integerPart}.${decimalDigit}`,
          unit: '秒',
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl bg-neutral-900 border border-neutral-800 p-5 sm:p-7 shadow-xl">
      ...
      {/* Top Section: Era & Age in Year/Month/Day/Hour/Minute/Second */}
      <div className="pb-6 border-b border-neutral-800 space-y-3">
        ...
        <div>
          <div className="text-xs text-neutral-400 font-mono uppercase tracking-wider mb-2 flex items-center gap-1.5">
      ...
      <div className="my-6">
        ...
        <div className="p-5 sm:p-7 rounded-xl bg-neutral-950 border border-neutral-800 shadow-inner">
          {selectedUnit === 'detailed' ? (
            ...
            /* Single unit format (Integer only) */
      ...
      <div className="space-y-2 pt-2">
        <div className="flex justify-between text-xs text-neutral-400">
          ...
          <span className="font-semibold text-amber-300 font-mono-numbers">
            現在: {integerAge}歳 (進行度: {metrics.percentageLived.toFixed(1)}% / 残り: {metrics.percentageRemaining.toFixed(1)}%)
          </span>
      ...
      {/* Micro Progress Bars: Today, This Year, Waking Time */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-5 mt-5 border-t border-neutral-800">
        ...
        <div className="p-3 rounded-xl bg-neutral-950 border border-neutral-800">
          <div className="flex items-center justify-between text-xs mb-1.5">
            <span className="text-neutral-400 flex items-center gap-1.5">
              <Zap className="w-3.5 h-3.5 text-emerald-400" /> 残りの覚醒活動日数
            </span>
